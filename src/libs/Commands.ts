...
import type {ThrottleFile} from '../providers/ThrottleLinkProvider'
...
export function registerCommands(getMiddleware: () => Promise<MiddlewareMap>): vscode.Disposable[] {
    return [
        ...
        vscode.commands.registerCommand('lgc.openLink', openLink),
    ]
}
...
async function showMiddleware(route: Route | {middleware: string}, getMiddleware: () => Promise<MiddlewareMap>) {
...
async function openLink(files: ThrottleFile[], query: string) {
    const file = files.length === 1
        ? files[0]
        : await chooseThrottleFile(files)
    ...
    await selectThrottleQuery(editor, query)
}

async function chooseThrottleFile(files: ThrottleFile[]): Promise<ThrottleFile | undefined> {
    const choice = await vscode.window.showQuickPick(files.map((file) => ({
        label : file.label,
        file,
...
async function selectThrottleQuery(editor: vscode.TextEditor, query: string) {
    const text = editor.document.getText()

    for (const match of text.matchAll(new RegExp(escapeStringRegexp(query), 'g'))) {
        const range = new vscode.Range(editor.document.positionAt(match.index), editor.document.positionAt(match.index + query.length))
